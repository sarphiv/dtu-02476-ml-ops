...
def load_model_best(cfg: Any) -> BaseModel:
    ...
    with open(Path(cfg.training.models.model_dir) / "best_model.yaml", "r") as file:
...
def transform_picture_to_data_point(img : Image.Image, cfg: Any) -> TensorType[1, channels, height, width]:
    ...
    """
    # Get the image size
    img_size = cfg.training.models.img_size
...
def predict_data_class(
    ...
    return model.idx_to_class[str(model(data).argmax(1).item())]
